...
def extract_features_from_definitions(style_defs: dict) -> set:
    features = set()
    for group in style_defs.values():
        for style in group.values():
            for rule in style["rules"]:
                features.add(rule["feature"])
    return features
    
def split_features_by_stem(features: dict) -> dict:
    """Split flat feature dict into {stem: {feature_name: value}}."""
    split = {}
    for k, v in features.items():
        ...
        if stem not in split:
            split[stem] = {}
        split[stem][feature] = v
    return split

def generate_feature_event_log_direct(snapped_time: float, window_s: float, feats: dict, post_feats: dict) -> list:
    """Generate feature events with exact stem and feature (no inference)."""
    features_used = extract_features_from_definitions(STYLE_DEFINITIONS)
    events = []

    pre_stem_feats = split_features_by_stem(feats)
    for stem, fdict in pre_stem_feats.items():
        for feature_name in fdict:
            full_name = f"{feature_name}_{stem}"
            if full_name in features_used:
                events.append({
                    "feature": feature_name,
                    "stem": stem,
                    "start": snapped_time - window_s,
                    "end": snapped_time
                })

    post_stem_feats = split_features_by_stem(post_feats)
    for stem, fdict in post_stem_feats.items():
        for feature_name in fdict:
            full_name = f"post_{feature_name}_{stem}"
            if full_name in features_used:
                events.append({
                    "feature": f"post_{feature_name}",
                    "stem": stem,
                    "start": snapped_time,
                    "end": snapped_time + window_s
                })
...
def process_track(track_path):
    ...
    hop_s = HOP_SIZE / SR
    data['energy_curves'] = {
        k: [[i * hop_s, float(val)] for i, val in enumerate(v)]
        for k, v in stem_curves.items()
    }
    ...
    for target_field in ["first_downbeat", "loop_start"]:
        ...
        # Inject feature_events into JSON
        window_s = window_frames * (HOP_SIZE / SR)
        feature_events = generate_feature_event_log_direct(
            target_field,
            snap_s,
            window_s,
            feats,
        ...
        data["features"][target_field]["feature_events"] = feature_events
    ...
    if 'features' in data and 'first_downbeat' in data['features']:
        post_feats = {k: v for k, v in data['features']['first_downbeat'].items() if k.startswith('post_')}
        print(f"First Downbeat Post → {evaluate_styles(post_feats, 'post')}")
    else:
        print("First Downbeat Post → [! skipped, missing input]")
...
def save_styles_to_csv(data, output_path):
    rows = []
    track_name = Path(data["track_path"]).name
    bpm = data.get("bpm", None)
    beat_frames = data.get("beat_frames", None)
    window_frames = 32 * beat_frames if beat_frames else None
    hop_s = HOP_SIZE / SR
    window_s = window_frames * hop_s if window_frames else None

    for label in ["first_downbeat", "loop_start"]:
        if label not in data.get("features", {}):
            continue

        snapped_time = data.get(f"{label}_snapped", None)
        feats = data["features"][label]
        post_feats = {k: v for k, v in feats.items() if k.startswith("post_")}
        pre_feats = {k: v for k, v in feats.items() if not k.startswith("post_")}
        ...
        row = {
            ...
            "feature_events": json.dumps(
                generate_feature_event_log_direct(snapped_time, window_s, pre_feats, post_feats)
            ),
            "styles": ", ".join(data.get("downbeat_style" if label == "first_downbeat" else "loop_intro_style", [])),
            "post_styles": ", ".join(data.get("loop_post_style", [])) if label == "loop_start" else ""
        }

        # Add pre/post energy curves (as stringified lists)
        for stem in EXPECTED_STEMS:
            curves = data.get("energy_curves", {}).get(stem, [])
            if not curves:
                row[f"pre_{stem}"] = ""
                row[f"post_{stem}"] = ""
                continue

            # Reconstruct numpy array from [timestamp, value] pairs
            arr = np.array([v for _, v in curves])
            snap_idx = int(round(snapped_time / hop_s))
            if not window_frames:
                row[f"pre_{stem}"] = ""
                row[f"post_{stem}"] = ""
                continue

            pre_start = max(0, snap_idx - window_frames)
            pre_end = snap_idx
            post_start = snap_idx
            post_end = min(len(arr), snap_idx + window_frames)

            row[f"pre_{stem}"] = json.dumps(arr[pre_start:pre_end].tolist())
            row[f"post_{stem}"] = json.dumps(arr[post_start:post_end].tolist())

        # Add all features (flatten)
        for k, v in feats.items():
            row[k] = v
...
def main():
    ...
    if base.is_dir():
        data = process_track(base)
        if data:
            csv_path = base / "downbeat_styles.csv"
            save_styles_to_csv(data, csv_path)
