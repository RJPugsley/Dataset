...
def run_pipeline(audio_file_path):
    ...
    try:
        track_folder = extract_stems(audio_file, output_root=Path("/Volumes/Dataset/Processed_files"))
        if not track_folder:
...
def run_in_parallel(file_list):
    ...
    print(f"📊 Detected {max_processes} logical cores. Running {max_processes} parallel processes.")
    with Pool(processes=max_processes) as pool:
        pool.map(run_pipeline, [str(f) for f in file_list])
